# PhD Hub

This is a GitHub-hosted recreation of your Notion-style PhD template interface.

## Open the interface
- [`index.html`](./index.html)

## Visit `index.html` using a URL
If this repo is on GitHub at `https://github.com/<owner>/<repo>`:

1. **File URL (view source on GitHub):**
   - `https://github.com/<owner>/<repo>/blob/<branch>/index.html`
2. **GitHub Pages URL (rendered webpage, recommended):**
   - Enable **Settings → Pages** for this repository.
   - Then visit: `https://<owner>.github.io/<repo>/`
   - Or directly: `https://<owner>.github.io/<repo>/index.html`

## Edit without CLI
1. Open any page from the dashboard.
2. Click **Edit this file** in GitHub.
3. Update content.
4. Commit directly in GitHub.

## Main databases
- [Tasks](./tasks/_index.md)
- [Projects](./research/projects-overview.md)
- [Meeting & Seminar Notes](./research/meeting-seminar-notes/_index.md)
- [Literature Notes](./research/literature-notes/_index.md)
- [Papers](./papers/papers-overview.md)
- [Dissertation](./dissertation/overview.md)
- [Publication Record](./career/publication-record.md)
- [Opportunities](./career/opportunities.md)
- [Professional Contacts](./career/professional-contacts.md)
